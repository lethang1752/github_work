...
union all
select 'restore from platform ''${src_platform_name}'' foreign datafile ' || file_id || ' format '''|| file_name ||''' from backupset ''${target_backup_dest}/' || lower(t.tablespace_name) || trim(to_char(row_number() over (partition by t.tablespace_name order by f.file_id),'00')) || '_${file_name}'';'
from dba_data_files f, dba_Tablespaces t
where f.tablespace_name=t.tablespace_name
and   t.CONTENTS='PERMANENT'
and   t.tablespace_name not in ('SYSTEM','SYSAUX')
